...
def build(year):
    ...
    for n, kind in [(47, "填充"), (48, "填充"), (50, "簡答")]:
        未納入互動.append({"題號": n, "題型": "混合題-非選擇(%s)" % kind, "原因": "非選擇題，不納入互動作答"})
    未納入互動.append({"題型": "第參部分-中譯英", "原因": "非選擇題，不納入互動"})
    未納入互動.append({"題型": "第參部分-英文作文", "原因": "作文題，不納入互動（僅保留官方 PDF 瀏覽）"})

    interactive = [q["題號"] for g in 題組 for q in g["題目"]] + [q["題號"] for q in 單題]
    ...
    result = {
        ...
        "統計": {"互動題數": len(interactive), "題組數": len(題組), "單題數": len(單題),
                "未納入互動": len(未納入互動)},
        "題組": 題組, "單題": 單題, "未納入互動": 未納入互動, "review_flags": review_flags,
    }
...
def build_mixed(year, cfg, lines, ans, question_pdf, img_dir, img_web, 題組, 未納入互動):
    ...
    if cfg["mixed"]["style"] == "image":
        ...
        題目 = [{"題號": 49, "作答模式": "多選", "題幹文字": q49_stem, "選項": opts, "答案": ans[49],
                "備註": "多選題"}]
        ...
        題目 = [{"題號": 49, "作答模式": "多選", "題幹文字": q49_stem, "choices": choices, "答案": ans[49],
...
def emit_app_bank(year, result, out_bank):
    ...
    bank = {"id": "english-%s-answer-card" % year, "subject": "英文", "year": year,
            ...
            "source": result["來源"],
            "note": "收錄 %s 學測英文選擇題共 %d 題，題目/選項/文章/答案均取自官方 PDF；非選擇題（填充、簡答、翻譯、作文）不納入。" % (
                year, result["統計"]["互動題數"]),
